  ...
  m_lac(args.lac)
{
  ...
  // srsran::console("NAS Context Initialized. MCC: 0x%x, MNC 0x%x\n", m_mcc, m_mnc);

  /** Init Contester ************************/
  srsran::console("*******************************************\n");
  srsran::console("***************** Contester ***************\n");
  srsran::console("*******************************************\n");
  m_contester_nas = contester_nas::get_instance();
  m_contester_nas->init();
}
                                                ...
                                                const nas_if_t&                                       itf)
{
  ...
  nas_ctx->m_contester_nas->init_sig.emit(nas_ctx, nas_tx.get(), s1ap);
                                 ...
                                 const nas_if_t&         itf)
{
  ...
  if (mac_valid) {
    ...
    nas_ctx->m_contester_nas->handle_sig.emit(LIBLTE_MME_SECURITY_HDR_TYPE_SERVICE_REQUEST, nas_ctx, NULL);
    ...
    nas_ctx->m_contester_nas->handle_sig.emit(LIBLTE_MME_SECURITY_HDR_TYPE_SERVICE_REQUEST, nas_ctx, nas_tx.get());
...
bool nas::handle_attach_request(srsran::byte_buffer_t* nas_rx)
{
  ...
  if (m_emm_ctx.imsi == 0) {
    ...
    m_contester_nas->init_sig.emit(this, nas_tx.get(), m_s1ap);
...
bool nas::pack_security_mode_command(srsran::byte_buffer_t* nas_buffer)
{
  ...
  m_contester_nas->set_parameter_before_pack_message(LIBLTE_MME_MSG_TYPE_SECURITY_MODE_COMMAND, &sec_hdr_type);
...
bool nas::pack_esm_information_request(srsran::byte_buffer_t* nas_buffer)
{
  ...
  m_contester_nas->set_parameter_before_pack_message(LIBLTE_MME_MSG_TYPE_ESM_INFORMATION_REQUEST, &sec_hdr_type);
  ...
  m_contester_nas->set_mac_after_pack_message(LIBLTE_MME_MSG_TYPE_ESM_INFORMATION_REQUEST, &mac);
  memcpy(&nas_buffer->msg[1], mac, 4);
...
bool nas::pack_identity_request(srsran::byte_buffer_t* nas_buffer)
{
  ...
  m_contester_nas->set_parameter_before_pack_message(LIBLTE_MME_MSG_TYPE_IDENTITY_REQUEST, &id_req);
...
bool nas::pack_emm_information(srsran::byte_buffer_t* nas_buffer)
{
  ...
  m_contester_nas->set_parameter_before_pack_message(LIBLTE_MME_MSG_TYPE_EMM_INFORMATION, &sec_hdr_type);
...
bool nas::pack_service_reject(srsran::byte_buffer_t* nas_buffer, uint8_t emm_cause)
{
  ...
  m_contester_nas->set_parameter_before_pack_message(LIBLTE_MME_MSG_TYPE_SERVICE_REJECT, &emm_cause);
...
bool nas::pack_tracking_area_update_reject(srsran::byte_buffer_t* nas_buffer, uint8_t emm_cause)
{
  ...
  m_contester_nas->set_parameter_before_pack_message(LIBLTE_MME_MSG_TYPE_TRACKING_AREA_UPDATE_REJECT, &emm_cause);
...
/************************
 *
 * New Functions for Contester
 *
 ************************/
bool nas::pack_guti_reallocation_command(srsran::byte_buffer_t* nas_buffer)
{
  LIBLTE_MME_GUTI_REALLOCATION_COMMAND_MSG_STRUCT guti_reall;
  ...
  guti_reall.guti.type_of_id        = LIBLTE_MME_EPS_MOBILE_ID_TYPE_GUTI;
  guti_reall.guti.guti.mcc          = mcc;
  guti_reall.guti.guti.mnc          = mnc;
  guti_reall.guti.guti.mme_group_id = m_mme_group;
  guti_reall.guti.guti.mme_code     = m_mme_code;
  guti_reall.guti.guti.m_tmsi       = m_s1ap->allocate_m_tmsi(m_emm_ctx.imsi);

  guti_reall.tai_list_present = true;
  guti_reall.tai_list.N_tais     = 1;
  guti_reall.tai_list.tai[0].mcc = mcc;
  guti_reall.tai_list.tai[0].mnc = mnc;
  guti_reall.tai_list.tai[0].tac = m_tac;
  
  uint8_t sec_hdr_type = LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED;

  m_contester_nas->set_parameter_before_pack_message(LIBLTE_MME_MSG_TYPE_GUTI_REALLOCATION_COMMAND, &sec_hdr_type);

  m_sec_ctx.dl_nas_count++;
  LIBLTE_ERROR_ENUM err = liblte_mme_pack_guti_reallocation_command_msg(
      &guti_reall, sec_hdr_type, m_sec_ctx.dl_nas_count, (LIBLTE_BYTE_MSG_STRUCT*)nas_buffer);
  if (err != LIBLTE_SUCCESS) {
    m_logger.error("Error packing GUTI Reallocation\n");
    srsran::console("Error packing GUTI Reallocation\n");
    return false;
  }
  ...
  uint8_t mac[4];
  integrity_generate(nas_buffer, mac);
  m_contester_nas->set_mac_after_pack_message(LIBLTE_MME_MSG_TYPE_GUTI_REALLOCATION_COMMAND, &mac);
  memcpy(&nas_buffer->msg[1], mac, 4);

  return true;
}

bool nas::pack_downlink_nas_transport_msg(srsran::byte_buffer_t* nas_buffer)
{
  LIBLTE_MME_DOWNLINK_NAS_TRANSPORT_MSG_STRUCT dl_nas_transport;
  uint8_t msg[] = {
      0x09, 0x01, 0x20, 0x01, 0x00, 0x08, 0x81, 0x68, 0x31, 0x10, 0x20, 0x00, 0x05, 0xf0, 0x00, 
      0x13, 0x04, 0x03, 0x81, 0x55, 0xf5, 0x00, 0x00, 0x22, 0x60, 0x22, 0x31, 0x70, 0x60, 0x00, 
      0x04, 0xf4, 0xf2, 0x9c, 0x0e};

  memcpy(dl_nas_transport.nas_msg.msg, msg, sizeof(msg));
  dl_nas_transport.nas_msg.N_bytes = sizeof(msg);
  
  uint8_t sec_hdr_type = LIBLTE_MME_SECURITY_HDR_TYPE_INTEGRITY_AND_CIPHERED;

  m_contester_nas->set_parameter_before_pack_message(LIBLTE_MME_MSG_TYPE_DOWNLINK_NAS_TRANSPORT, &sec_hdr_type);
  ...
  if (err != LIBLTE_SUCCESS) {
    m_logger.error("Error packing Downlink NAS Transport Msg\n");
    srsran::console("Error packing Downlink NAS Transport Msg\n");
    return false;
  }
  // Encrypt NAS message
  cipher_encrypt(nas_buffer);
...
bool nas::pack_attach_reject(srsran::byte_buffer_t* nas_buffer, uint8_t emm_cause)
{
  ...
  m_contester_nas->set_parameter_before_pack_message(LIBLTE_MME_MSG_TYPE_ATTACH_REJECT, &emm_cause);

  attach_rej.esm_msg_present = false;

  attach_rej.t3446_value_present = true;
  attach_rej.t3446_value         = 30;

  attach_rej.emm_cause = emm_cause;

  LIBLTE_ERROR_ENUM err = liblte_mme_pack_attach_reject_msg(
      &attach_rej,  (LIBLTE_BYTE_MSG_STRUCT*)nas_buffer);
